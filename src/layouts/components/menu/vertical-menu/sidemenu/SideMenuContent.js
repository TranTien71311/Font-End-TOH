  ...
  render() {
    ...
    const menuItems = navigationConfig.map(item => {
      ...
      if (item.type === "groupHeader") {
        if((item.permissions &&
          this.props.currentUser.includes(item.permissions[0])) ||
      ...
      let renderItem = (
        <li
          ...
          onClick={e => {
            ...
            this.props.updateShowCart(item.showCart)
          }}>
          <CustomAnchorTag
            to={
              item.filterBase
      ...
      if (
        (item.type === "collapse" && this.props.currentUser.includes(item.permissions[0])) ||
...
export default connect(mapStateToProps, {updateShowCart })
(SideMenuContent)
