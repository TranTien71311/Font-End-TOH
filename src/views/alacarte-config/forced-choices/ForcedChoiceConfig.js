...
import {
  ...
  filterData,
  updateData
} from "../../../redux/actions/alacarte-config/forced-choice"
import Sidebar from "./ForcedChoiceSidebar"
...
class ForcedChoiceConfig extends Component {
    ...
    columns: [
      {
        ...
        minWidth: "300px",
        cell: row => (
          <p title={row.ChoiceProductName} className="text-truncate text-bold-500 mb-0">
            {row.ChoiceProductName}
          </p>
        )
      },
      {
        ...
        minWidth: "30px",
        cell: row => (
          <p title={row.Descript} className="text-truncate text-bold-500 mb-0 text-right">
            {row.Descript}
          </p>
        )
      },
      {
        ...
        minWidth: "30px",
        cell: row => (
          <p title={row.Question} className="text-truncate text-bold-500 mb-0 text-right">
            {row.Question}
          </p>
        )
      },
      {
        ...
        minWidth: "30px",
        cell: row => (
          <p title={row.Price} className="text-truncate text-bold-500 mb-0">
            {row.Price}
          </p>
        )
      },
  ...
  static getDerivedStateFromProps(props, state) {
    if (
      props.focedChoices.data.length !== state.data.length ||
      state.currentPage !== props.parsedFilter.page
    ) {

      return {
        data: props.focedChoices.data,
        allData: props.focedChoices.filteredData,
        totalPages: props.focedChoices.totalPages,
        currentPage: parseInt(props.parsedFilter.page) - 1,
        rowsPerPage: parseInt(props.parsedFilter.perPage),
        totalRecords: props.focedChoices.totalRecords,
        sortIndex: props.focedChoices.sortIndex,
        statusAPI: props.focedChoices.statusAPI,
        showModalApi: props.focedChoices.showModalApi,
        messageAPI: props.focedChoices.messageAPI,
        timeAPI: props.focedChoices.timeAPI,
        translations: props.focedChoices.translations
      }
    }
...
const mapStateToProps = state => {
  ...
  return {
    focedChoices: state.dataPOSForcedChoice
  }
}
